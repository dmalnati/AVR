...
// Didn't re-implement to test savings, but I'm sure I can do something here
...
// Didn't re-implement to test savings, but I'm sure I can do something here
// looks like inner if is atoi, but trying to make sure the end result is in
// the right units, so starts by assuming it's in 10,000ths.
...
bool TinyGPS::term_complete()
{
  ...
  if (_term_number == 0)
  {
    if (!gpsstrcmp(_term, "GPRMC") || !gpsstrcmp(_term, "GNRMC"))
      _sentence_type = _GPS_SENTENCE_GPRMC;
    else if (!gpsstrcmp(_term, "GPGGA") || !gpsstrcmp(_term, "GNGGA"))
...
// save 84 bytes if you can just replace with atol()
